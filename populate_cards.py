...
def parse_card(global_id):
    ...
    if archive_soup and archive_soup.select(".userLeft") and archive_soup.select(".window3"):
        ...
        print repr(archive_soup.select(".window3 span")[0]).get_text()
        name = name_rgx.search(archive_soup.select(".window3")[0].find_next_sibling(text="(").encode('utf-8').strip(), re.MULTILINE).group(1)
        name_prefix_match = prefix_rgx.search(name)
        if name_prefix_match:
            name_prefix = name_prefix_match.group(1).encode('utf-8').strip()
            root_name = re.search(name_prefix + r"([\w\d\s\n-.]*)\+*", name).group(1).lower().replace(" ", "")
        ...
        faction = re.search(r"Faction:\s([\w\d\s]+)", archive_soup.select(".userLeft p")[1].get_text().encode('utf-8').strip()).group(1)
                        ...
                        for effect_sibling in effect_item.find_parent("table").find_all("td"):
                            if effect_sibling.name == "td":
                                ...
                                if effect_match:
                                    ...
                                    #parse out effect properties from effecrt verbiage
                                    effect_strength = eff_strength_rgx.search(effect)
                                    effect_cards_affected = eff_cards_rgx.search(effect)
                                    effect_attribute = eff_attr_rgx.search(effect)
    ...
    if card_exists and archive_soup and rally_soup:
        print '\t'.join([global_id, name, name_prefix, root_name, image_id, rarity, alignment, min_rarity, base_id, v, desc, base_atk, base_def, pwr_req, gender, faction, silver, max_level, max_mastery,  ability, usage, effect, effect_strength, effect_cards_affected, effect_attribute, effect_player, effect_direction])
...
if __name__ == "__main__":
    print '\t'.join(["global_id", "name", "name_prefix", "root_name", "image_id", "rarity", "alignment", "min_rarity", "base_id", "version", "desc", "base_atk", "base_def", "pwr_req", "gender", "faction", "silver", "max_level", "max_mastery", "ability", "usage", "effect_verbiage", "effect_strength", "effect_cards_affected", "effect_attribute", "effect_player", "effect_direction"])
