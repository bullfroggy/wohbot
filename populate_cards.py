...
def parse_card(global_id):
    ...
    if archive_soup and archive_soup.select(".userLeft") and archive_soup.select(".window3"):
        ...
        root_name = re.search(r"(.*)\+*", root_name)
        ...
        if ability_found:
            ...
            ability_match = re.search(r"Ability:\s([.*?]+)\sUsage", ability_found.find_parent("div").get_text().strip())
            if ability_match:
                    ...
                    for effect_item in effect_found:
                        for effect_sibling in effect_item.find_parent("table").find_all("td"):
                            if effect_sibling.name == "td":
                                ...
                                if effect_match:
                                    effect = unicode(effect_match).strip()
...
if __name__ == "__main__":
    f.write('\t'.join(["global_id", "name", "name_prefix", "root_name", "image_id", "rarity", "alignment", "min_rarity", "base_id", "version", "desc", "base_atk", "base_def", "pwr_req", "gender", "faction", "silver", "max_level", "max_mastery", "ability", "usage", "effect_verbiage", "effect_strength", "effect_cards_affected", "effect_attribute", "effect_player", "effect_direction"]))
