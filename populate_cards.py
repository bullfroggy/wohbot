...
def process_parent(card_id):
    v = parse_card(card_id)
    # TODO build a list of siblings to process based on v
    siblings = []
...
def parse_card(global_id):
    ...
    if card_exists:
        print '\t'.join([global_id, name, name_id, name_prefix, image_id, rarity, alignment, min_rarity, base_id, desc, base_atk, base_def, pwr_req, gender, faction, silver, ability, usage, effect, max_level, max_mastery])
