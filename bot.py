from card, player import Card, Player
...
class Bot:
	...
	def get_card_count(self):
		mypage_url = self.get_url("mypage")
		html = self.parse_page(mypage_url)
		if html:
	...
	def update_card_ids(self):
		card_ids = [""]
		card_list_urls = [self.get_url("card_list")+str(page) for page in range(0, self.get_card_count(), 10)]
	...
	def farm(self, iterations):
		mission_url = self.get_url("mission-%s" %self.settings["farm_mission"])
